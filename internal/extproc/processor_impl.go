...
func (r *routerProcessor[ReqT, RespT, RespChunkT, EndpointSpecT]) processListFilesRequestHeaders(ctx context.Context) (*extprocv3.ProcessingResponse, error) {
	...
	r.originalModel = ""
	stickyBackendMetadata := buildStickyBackendDynamicMetadata(backendName)
	...
	return &extprocv3.ProcessingResponse{
		...
		DynamicMetadata: stickyBackendMetadata,
	}, nil
}
...
func (r *routerProcessor[ReqT, RespT, RespChunkT, EndpointSpecT]) processFileIDInPathRequestHeaders(ctx context.Context) (*extprocv3.ProcessingResponse, error) {
	...
	// Keep backend name in processor state for dynamic metadata affinity matching
	// and file ID routing context.
	if backendName != "" {
		r.requestHeaders[internalapi.BackendNameHeaderKey] = backendName
	}
...
func (u *upstreamProcessor[ReqT, RespT, RespChunkT, EndpointSpecT]) ProcessRequestHeaders(ctx context.Context, _ *corev3.HeaderMap) (res *extprocv3.ProcessingResponse, err error) {
	...
	dm = mergeDynamicMetadata(dm, buildStickyBackendDynamicMetadata(u.requestHeaders[internalapi.BackendNameHeaderKey]))
	return &extprocv3.ProcessingResponse{
		Response: &extprocv3.ProcessingResponse_RequestHeaders{
			RequestHeaders: &extprocv3.HeadersResponse{
				Response: &extprocv3.CommonResponse{
...
// Returns format "namespace.name" to match selected_backnd dynamic metadata affinity matching.
...
func (u *upstreamProcessor[ReqT, RespT, RespChunkT, EndpointSpecT]) SetBackend(ctx context.Context, backend *filterapi.RuntimeBackend, routeName string, routeProcessor Processor) (err error) {
	...
	// Keep namespace-qualified AIServiceBackend name (namespace.name format) in
	// processor state for selected_backnd dynamic metadata and file ID encoding.
...
func buildStickyBackendDynamicMetadata(stickyBackend string) *structpb.Struct {
	if stickyBackend == "" {
		return nil
	}
	return &structpb.Struct{
			...
			internalapi.AIGatewayFilterMetadataNamespace: {
				Kind: &structpb.Value_StructValue{
					StructValue: &structpb.Struct{Fields: map[string]*structpb.Value{
						internalapi.AIGatewaySelectedBackndMetadataKey: {
							Kind: &structpb.Value_StringValue{StringValue: stickyBackend},
						},
					}},
				},
			},
