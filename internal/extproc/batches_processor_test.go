...
func TestBatchProcessResponseBody_ReEncodeResponse(t *testing.T) {
	...
	// Simulate the gateway file ID the client originally supplied in the create request. The
	// processor stores it in gatewayInputFileID and must echo it verbatim in the response.
	originalGatewayFileID := mustEncodeFileID(t, codec, "ns", "apple", "file-input-native")
	...
	p := &batchesProcessor{
		...
		gatewayInputFileID: originalGatewayFileID,
	}
	...
	// input_file_id must be the EXACT original gateway ID the client sent — not a fresh re-encoding.
	require.Equal(t, originalGatewayFileID, gjson.GetBytes(mutated, "input_file_id").String(),
		"input_file_id in create response must match the original gateway file ID the client sent")

	// output_file_id and error_file_id are IDs the gateway has never seen before (generated by
	// the upstream during batch processing); they must still be freshly gateway-encoded.
	for _, field := range []string{"output_file_id", "error_file_id"} {
...
// mustEncodeFileID encodes a native file ID into a gateway file ID for the given backend.
func mustEncodeFileID(t *testing.T, codec idcodec.Codec, namespace, name, nativeID string) string {
	t.Helper()
	gw, err := codec.Encode(idcodec.BackendID{
